...
struct Material {
   ...
   // sampler2D texture_diffuse7; //reflection
   // sampler2D texture_diffuse8; //refraction
...
//	result += emission;
    ...
    FragColor = vec4(1.0,1.0,1.0,0.2); //result;
...
vec4 CalculateTexture()
{
    ...
    terrainSampler[5] = texture(material.texture_diffuse6, vec2(TexCoords.x, TexCoords.y));

   // vec4 reflectionColour = texture(material.texture_diffuse7, TexCoords);
   // vec4 refractionColour = texture(material.texture_diffuse8, TexCoords);
   //
   // vec4 waterMap = mix(reflectionColour, refractionColour, 0.5);
    ...
    if(terrainSampler[0].r == 0.0 
        ...
        if( terrainSampler[5].a <= 0.0 ) { discard; }
            ...
            mixture = vec4(1.0, 1.0, 1.0, terrainSampler[5].a / 5.0);
        }
    }
